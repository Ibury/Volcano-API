...
from api.shared.tools import dir_files_list  # Utility functions shared across the project
from api.shared.xlsx_parser import parse_xlsx, map_name_list  # Functions to parse Excel files and list map names
import logging
...
def get_file(file_name_no_ext, filetype, timestamp=None):
    ...
    """
    base_path = f'/var/www/geoinquire/volcanoes/{file_name_no_ext}'
    try:
...
def get_overlay(volcano, hazard):

    base_path = (f"/var/www/geoinquire/volcanoes/{volcano}")

    metadata_path = os.path.join(base_path, "metadata",f"{hazard}.xlsx")

    metadata = parse_xlsx(metadata_path)

    box = metadata.get("Box")

    png_url = metadata.get("Png file")

    if not box:
        abort(
            404,
            f"Box not found in {hazard}.xlsx"
        )

    if not png_url:
        abort(
            404,
            f"Png file not found in {hazard}.xlsx"
        )
...
def get_download_file(volcano, filename):

    if not filename.endswith(".zip"):
        filename = f"{filename}.zip"

    filepath = os.path.join(
        "/var/www/geoinquire/volcanoes",
        volcano,
        "downloads",
        filename
    )
...
def get_legend_file(volcano, hazard):

    filepath = os.path.join(
        "/var/www/geoinquire/volcanoes",
        volcano,
        "imgs",
        f"legend_{hazard}.png"
    )
