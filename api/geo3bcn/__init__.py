...
from api.geo3bcn.serializers import map_summary, file_name, target, metadata, volcano, overlay
from api.restx import api

# Import helper functions for data retrieval and file serving
from api.geo3bcn.helpers import get_volcanoes_summary, get_event_tree_metadata, get_map_metadata, get_metadata, \
    get_maps_summary, get_file, get_overlay, get_download_file, get_legend_file
...
# Modificarlo para que dado un volcan devuelva los mapas
...
@ns.route('/overlay/<string:volcano>/<string:hazard>')
class OverlayResource(Resource):

    @ns.marshal_with(overlay)
    def get(
        self,
        volcano,
        hazard
...
@ns.route('/download/<string:volcano>/<string:filename>')
...
@ns.route('/legend/<string:volcano>/<string:hazard>')
