...
def get_map_summary(current_path, volcanoes_path, volcano):
    """
   ...
   """
    try:
        file_name, _ = os.path.splitext(volcano)  # Remove file extension
        path = os.path.join(current_path, volcanoes_path, volcano, "metadata")  # Construct path to metadata
        response = {"volcano_target": volcano, "data": xlp.map_name_list(path)}  # Generate response
        return response
    except Exception as e:
        log.error(f"Error fetching map summaries for {volcano}: {e}")
        return {"error": f"Unable to fetch map summaries for {volcano}."}